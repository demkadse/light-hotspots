...
import { hashDiscordUserId, rememberTemplateOwner } from "./identityService.js";
...
function buildOwnerHash(value) {
  if (/^\d{17,20}$/.test(String(value || ""))) {
    return hashDiscordUserId(value);
  }

  return "legacy-non-discord";
}

export async function migrateTemplatePrivacyData() {
  ...
  return changed ? [TEMPLATE_PATH] : [];
}

export async function migrateAuditPrivacyData() {
  ...
  return changed ? [AUDIT_LOG_PATH] : [];
}

export async function migrateLegacyEventsPrivacyData() {
  ...
  return changed ? [LEGACY_EVENTS_PATH] : [];
}

export async function migratePublishedEventsPrivacyData() {
  const changedFiles = [];
...
export async function migrateAllPrivacyData() {
  const changedFiles = [
    ...(await migrateTemplatePrivacyData()),
    ...(await migrateAuditPrivacyData()),
    ...(await migrateLegacyEventsPrivacyData()),
    ...(await migratePublishedEventsPrivacyData())
  ];
